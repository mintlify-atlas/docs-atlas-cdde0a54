...
description: "Ladybird's from-scratch cryptographic primitives, TLS 1.2/1.3 implementation, and HTTP/1.1 client — all built independently without OpenSSL or other crypto libraries."
---
...
<Note>
LibCrypto, LibTLS, and LibHTTP are written entirely from scratch and are **not** wrappers around OpenSSL or any other third-party cryptography library. All algorithm implementations live in the Ladybird source tree. There is an `OpenSSL.h` / `OpenSSLForward.h` shim present in LibCrypto and LibTLS, but it exists only to provide compatibility hooks — the core algorithm implementations are independent.
</Note>
